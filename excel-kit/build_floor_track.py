#!/usr/bin/env python3
"""Build one thin Sassy_Closet_Track.xlsx from the live site export.

Floor tracker for teammates (Track / Orders / Readme). Not a Boss data dump.
Cute / pink / emoji / phone Excel themes are retired.
Website stays GF intake. Square Free remains on-hand inventory truth.
Never invent mã. No embeds. No Square Save. No Facebook Post.

Usage:
  python3 excel-kit/build_floor_track.py --out-dir ./out
  python3 excel-kit/build_floor_track.py --from-csv path.csv --out-dir ./out
"""
...
def fetch_export_csv(url: str) -> str:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "sassy-closet-excel-kit/floor-track",
        },
        method="GET",
    )
    try:
...
def flag_from_export(raw: dict[str, str]) -> str | None:
    ...
    explicit = text_or_none(raw.get("flag"))
    if explicit:
        return explicit
    return None
...
def map_export_row(raw: dict[str, str]) -> dict[str, object] | None:
    ...
    return {
        ...
        "photo_folder": photo_folder_link(ma),
...
def write_track(ws: Worksheet, rows: list[dict[str, object]]) -> None:
    ws.title = "Track"
    for col, name in enumerate(TRACK, start=1):
        ws.cell(1, col, name)
    ...
    for row_index, item in enumerate(rows, start=2):
        for col, name in enumerate(TRACK, start=1):
            ...
            if name in NUMBER_HEADERS and isinstance(item.get(name), (int, float)):
                ...
                cell.number_format = FORMAT_TEXT
    style_plain_header(ws, TRACK)
    last_row = max(len(rows) + 1, 1)
    ws.auto_filter.ref = f"A1:{get_column_letter(len(TRACK))}{last_row}"
    apply_named_widths(ws, TRACK, TRACK_WIDTHS)


def write_orders(ws: Worksheet) -> None:
    ws.title = "Orders"
    for col, name in enumerate(TRACK_ORDERS, start=1):
        ws.cell(1, col, name)
    for row in range(2, PLAIN_TEMPLATE_ROWS + 2):
        for col in range(1, len(TRACK_ORDERS) + 1):
            cell = ws.cell(row, col, None)
            cell.font = PLAIN_FONT
            cell.alignment = Alignment(vertical="center")
    style_plain_header(ws, TRACK_ORDERS)
    last = PLAIN_TEMPLATE_ROWS + 1
    ws.auto_filter.ref = f"A1:{get_column_letter(len(TRACK_ORDERS))}{last}"
    apply_named_widths(ws, TRACK_ORDERS, ORDERS_WIDTHS)
    add_list_dropdown(
        ws,
        header_index(TRACK_ORDERS, "pay") + 1,
        PLAIN_ORDER_PAY,
        start_row=2,
        end_row=last,
        prompt="pay",
    ...
    add_list_dropdown(
        ws,
        header_index(TRACK_ORDERS, "status") + 1,
        PLAIN_ORDER_STATUS,
        start_row=2,
        end_row=last,
        prompt="status",
...
def write_readme(ws: Worksheet) -> None:
    ...
    for row, line in enumerate(TRACK_README_LINES, start=1):
...
def verify_track_book(path: Path, *, expected_mas: Iterable[str] | None = None) -> list[str]:
    unzip_test(path)
    workbook = load_workbook(path)
    notes: list[str] = []
    try:
        if list(workbook.sheetnames) != list(TRACK_SHEETS):
            raise AssertionError(f"sheets must be {list(TRACK_SHEETS)}, got {workbook.sheetnames}")
        ...
        headers = [track.cell(1, col).value for col in range(1, len(TRACK) + 1)]
        if headers != list(TRACK):
            raise AssertionError(f"Track headers drifted: {headers}")
        if track.freeze_panes != "A2":
            raise AssertionError(f"Track freeze_panes={track.freeze_panes}")
        if not track.auto_filter.ref or not str(track.auto_filter.ref).startswith("A1:"):
            raise AssertionError(f"Track autofilter={track.auto_filter.ref}")
        if track.merged_cells.ranges:
            raise AssertionError(f"Track merged cells forbidden: {track.merged_cells.ranges}")

        mas = column_values(track, TRACK, "ma")
        if expected_mas is not None:
            want = [str(ma).strip() for ma in expected_mas]
            if list(mas) != want:
                raise AssertionError(f"Track mã mismatch: {list(mas)} != {want}")
        ...
        folders = column_values(track, TRACK, "photo_folder")
        for ma, folder in zip(mas, folders, strict=False):
            ...
            if not folder.endswith(f"Photos/{ma}/"):
                raise AssertionError(f"photo_folder must end Photos/{ma}/, got {folder!r}")
        if len(folders) != len(mas):
            raise AssertionError("every Track row needs a photo_folder")

        orders = workbook["Orders"]
        order_headers = [orders.cell(1, col).value for col in range(1, len(TRACK_ORDERS) + 1)]
        if order_headers != list(TRACK_ORDERS):
        ...
        if nonempty_row_count(orders, TRACK_ORDERS) != 0:
        ...
        if not (4 <= len(lines) <= 5):
            raise AssertionError(f"Readme must be 4–5 lines, got {len(lines)}: {lines}")
        ...
        notes.append(f"track_rows {len(mas)}")
...
def build(out_dir: Path, *, from_csv: Path | None = None, export_url: str = PLAIN_EXPORT_URL) -> Path:
    ...
    workbook.save(path)
    workbook.close()
    notes = verify_track_book(path, expected_mas=[str(row["ma"]) for row in rows])
    ...
    print(f"sync target: OneDrive {ONEDRIVE_TRACK}")
    print("floor tracker for teammates; cute Excel is retired")
    return path
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path("out"),
        help="folder for Sassy_Closet_Track.xlsx (default: ./out)",
    )
    parser.add_argument(
        "--from-csv",
        type=Path,
