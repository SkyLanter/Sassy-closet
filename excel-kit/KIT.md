...
| `INTAKE_DATASET_SYNC_WEBHOOK_URL` | Grok Bot routine `intake-dataset-sync-webhook` POST URL. Empty → no-op. Immediate path; poll backup already exists on Grok Bot so Mini Boss does not need to be online |
| `INTAKE_DATASET_SYNC_WEBHOOK_KEY` | Sender key as `Authorization: Bearer <key>` (optional `X-Automation-Key`). Both vars required or no-op. Boss pastes into Vercel **intake only**, then Redeploy intake |
