...
**ONE hub:** `sassycloset`. Lands at `Documents/Sassy Closet/sassycloset.xlsx` (`Photos/{MA}/`, `README.txt` in the same shop folder).

```bash
./kit.sh save     # hub from live export
./kit.sh books    # empty Square.xlsx + Finance.xlsx
```
...
| Path | Job |
| --- | --- |
| `excel-kit/KIT.md` | `kit.sh books` + `kit.sh save` |
| `excel-kit/build_square_finance.py` | Empty `out/Square.xlsx` + `out/Finance.xlsx` |
| `excel-kit/build_sassycloset_hub.py` | Live export → `out/sassycloset.xlsx` + `out/Photos/{ma}/` |
| `excel-kit/schema.py` | Headers, mã, Dashboard `B43`, Square import columns, FINANCE_* / HUB_* |
